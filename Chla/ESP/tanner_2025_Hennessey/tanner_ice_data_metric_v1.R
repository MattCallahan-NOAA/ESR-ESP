...
is2<-subset(is2,bsierp_id %in% c('3','4','5','6','8')  )
...
is_sub2<- is2 %>% group_by(year ) %>% summarise(avg_iceall = mean(ice_retr_roll15 ,na.rm=TRUE),
                                                         sd_iceall = sd(ice_retr_roll15 ,na.rm=TRUE))
head(is_sub2)
...
tanner_ice_allregions<-ggplot()+
  geom_point(data=is_sum,aes(x=year, y=avg_iceret))+
  geom_line(data=is_sum,aes(x=year, y=avg_iceret))+
...
write.csv(is_sum, file='Chla/ESP/tanner_2025_Hennessey/iceretreat_eachbsierpregionTanner_ShannonHennessey.csv')




write.csv(is_sub2, file='Chla/ESP/tanner_2025_Hennessey/iceretreat_average_allregions_ShannonHennessey.csv')

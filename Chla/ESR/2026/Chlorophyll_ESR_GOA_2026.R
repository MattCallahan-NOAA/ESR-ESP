...
plotdat <- data %>% 
  ...
  mutate(iseven=ifelse(year%in%(seq(1998,2026,by=2)),"even","odd")) %>% # Create an even/odd year flag for salmon models
  data.frame
...
png("ESR/2026/Figure_1_GOA.png",width=8,height=6,units="in",res=300)
plotdat %>% 
  ggplot() + 
  ...
  scale_y_continuous(limits=c(0,2.3),expand = c(0,0.0)) # Specify a boundary and expansion to set bars on bottom black line
dev.off()


png("ESR/2026/Figure_2_GOA.png",width=8,height=6,units="in",res=300)
data %>% 
  #filter(month%in%(3:6)) %>% 
...
png("ESR/2026/Chla_annual_lines.png",width=7,height=5,units="in",res=300)
ggplot() +
...
png("ESR/2026/Chla_annual_lines_smoothed.png",width=7,height=5,units="in",res=300)
ggplot() +
...
# plot all years
png("ESR/2026/Chla_yearbyyear_lines.png",width=7,height=20,units="in",res=300)
ggplot() +
  geom_line(data=data,
...
data %>% 
  filter(doy%in%c(117,109) & year==current.year) %>% 
  group_by(ecosystem_subarea) %>% 
  mutate(mymax=doy[meanchla==max(meanchla)][1])
...
data%>%
  ...
  print(n=Inf) %>%
  # save csv
  write.csv("ESR/2026/occci_conc_peak.csv",row.names=FALSE)
...
# plot
png("ESR/2026/chla_timeseries.png",width=7,height=5,units="in",res=300)
data%>%
  filter(month %in% (4:6))%>%
  group_by(year,ecosystem_subarea)%>%
  summarize(annual_meanchla=mean(meanchla)) %>%
...
png("ESR/2026/chla_coverage.png",width=7,height=5,units="in",res=300)
data %>%
  filter(month %in% (4:6) & year==current.year)%>%
  ggplot()+
  geom_line(aes(x=mid_date, y=coverage)) +
